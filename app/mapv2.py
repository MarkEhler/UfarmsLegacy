    ...
    for (i, row) in df.iterrows():
        if row['host'] == 1:
            ...
            if row.loc['contact'] == "m.ehler@comcast.net":
                ...
                <body>
                    <div>
                        <h3><a class="btn btn-default" onclick="redirectToURL(profile)">""" + f"""</i><span id='HostName'> {row['name']} </span></a></h3>
                        <h4>Contact: <a data-auto-recognition="true" href="mailto:zuliascleaning@gmail.com" class="wixui-rich-text__text"><span id='textToCopy'> {row.loc['contact']} </span></a>
